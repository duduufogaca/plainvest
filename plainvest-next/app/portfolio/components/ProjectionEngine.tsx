...
export function ProjectionEngine({ currentValue, monthlyContribution, currency, lang = 'en' }: Props) {
  ...
  return (
    <section className="portfolio-card proj-card">
      ...
      <h2>{isEN ? 'Where is your portfolio headed?' : 'Para onde vai seu portfólio?'}</h2>
      ...
      <div className="proj-controls">
        <div className="proj-control-group">
          ...
          <div className="proj-slider-row">
            <span className="proj-rate-display">{rate}%</span>
            <input type="range" min={1} max={20} step={0.5} value={rate}
              onChange={e => setRate(Number(e.target.value))} className="proj-slider" />
          </div>
        </div>
        <div className="proj-control-group">
          <span className="proj-control-label">{isEN ? 'Inflation' : 'Inflação'}</span>
          <div className="proj-slider-row">
            <span className="proj-rate-display">{inflation}%</span>
            <input type="range" min={0} max={10} step={0.5} value={inflation}
              onChange={e => setInflation(Number(e.target.value))} className="proj-slider" />
          </div>
        </div>
        <button className={`proj-real-btn${showReal ? ' proj-real-active' : ''}`}
          onClick={() => setShowReal(v => !v)}>
          {isEN ? 'Inflation-adjusted' : 'Ajustado à inflação'}
        </button>
      </div>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMinYMin meet"
          ...
          <defs>
            <linearGradient id="pg1" x1="0" y1="0" x2="0" y2="1">
              ...
              <stop offset="100%" stopColor="#61d5b4" stopOpacity="0.01" />
            </linearGradient>
          ...
          {yLabels.map((l, i) => (
            <g key={i}>
              ...
              <text x={PL - 5} y={l.y + 3.5} textAnchor="end"
                fontSize="8.5" fill="#253545" fontFamily="inherit">
                {fmtShortLabel(l.val)}
              </text>
            </g>
          ))}
          ...
          {mLines.map((m, i) => (
            <g key={i}>
              <line x1={PL} x2={W - 6} y1={m.y} y2={m.y}
                stroke="rgba(244,200,106,.18)" strokeWidth="1" strokeDasharray="4 5" />
              <text x={W - 4} y={m.y - 3} textAnchor="end"
                fontSize="7.5" fill="rgba(244,200,106,.45)" fontFamily="inherit">
                {fmtShortLabel(m.val)}
              </text>
            </g>
          ))}

          {/* Inflation-adjusted area + line */}
          {showReal && (
            <>
              <path d={realArea} fill="url(#pg2)" />
              <path d={realPath} fill="none" stroke="#818cf8" strokeWidth="1.5"
          ...
          <path d={nomPath} fill="none" stroke="#61d5b4" strokeWidth="2"
            strokeLinecap="round" strokeLinejoin="round" />

          {/* Current value dot */}
          <circle cx={toX(0)} cy={toY(CV)} r={3.5} fill="#61d5b4" />
          <circle cx={toX(0)} cy={toY(CV)} r={6} fill="rgba(97,213,180,.15)" />
      ...
      {/* Summary horizon cards */}
      <div className="proj-horizon-row">
        {projections.map(p => (
          <div key={p.years} className="proj-horizon-card">
            <span className="proj-horizon-years">{p.years}{isEN ? 'Y' : 'A'}</span>
        ...
        {milestoneData.map((m, i) => (
          <div key={i} className="proj-milestone-row">
            ...
            <div className="proj-milestone-bar-bg">
              <div className="proj-milestone-bar-fill" style={{ width: `${m.pct}%` }} />
            </div>
          </div>
        ))}
      </div>
