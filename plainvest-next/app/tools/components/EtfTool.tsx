...
export function EtfTool({ currency, rates, lang }: Props) {
  const t = TOOLS_T[lang]; const s = t.etf;
    ...
    <div className="tool-view">
      <div className="tool-grid">
        <div className="tool-inputs portfolio-card">
          <label className="tool-field"><span>{s.lStart} ({currency})</span>
            <input type="number" min={0} value={start} onChange={e => setStart(+e.target.value || 0)} /></label>
        ...
        <div className="tool-result-col">
          <div className="tool-result portfolio-card">
            ...
            <span className="tool-result-sub">{t.common.invested} {fmtCurrency(contributed, currency)} · {s.rGrowth} {fmtCurrency(Math.max(0, value - contributed), currency)}</span>
          </div>
