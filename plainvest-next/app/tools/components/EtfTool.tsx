...
export function EtfTool({ currency, rates, lang }: Props) {
  const T = TOOLS_T[lang]; const s = T.tool.etf; const co = T.common;
  const r = rates[currency] || 1;
  const [start, setStart] = useState(roundNice(convert(2000, r)));
      ...
      <div className="tool-grid">
        ...
        <div className="tool-result-col">
          ...
          <div className="portfolio-card tool-summary">
            <span className="tool-summary-title">{s.summaryTitle}</span>
            <StatRows items={[
              { label: s.initial, value: fmtCurrency(start, currency) },
              { label: s.monthlyInv, value: fmtCurrency(monthly, currency) },
              { label: s.totalInvested, value: fmtCurrency(contributed, currency) },
              { label: s.growthGen, value: fmtCurrency(growth, currency), strong: true },
            ]} />
            <p className="tool-note">{s.divNote}</p>
          </div>
