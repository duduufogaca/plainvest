...
export async function GET(request: Request) {
  ...
  const templates: TemplateMap = {
    ...
    newsletter:            { subject: "You're subscribed to Plainvest Insights",       html: T.newsletterConfirmationEmail(to) },
    ...
    password_changed:      { subject: 'Your password was updated',                     html: T.passwordChangedEmail(MOCK.name) },
    pro_welcome:           { subject: 'Welcome to Plainvest Pro',                      html: T.proWelcomeEmail(MOCK.name, to) },
    lifetime_welcome:      { subject: 'Welcome to Plainvest Lifetime',                 html: T.premiumWelcomeEmail(MOCK.name, to) },
    ...
    zoom_confirmation:     { subject: 'Your Plainvest support call is confirmed',      html: T.supportCallConfirmationEmail(MOCK.name, to, { date: MOCK.date, time: MOCK.time, meetingLink: MOCK.meetingLink }) },
    zoom_reminder:         { subject: 'Reminder: your Plainvest support call',         html: T.supportCallReminderEmail(MOCK.name, { date: MOCK.date, time: MOCK.time, meetingLink: MOCK.meetingLink }) },
    new_guide:             { subject: 'New guide available on Plainvest',              html: T.newGuideEmail(MOCK.name, { title: MOCK.guideTitle, description: MOCK.guideDesc, url: MOCK.guideUrl }) },
    milestone:             { subject: "You've reached a new milestone",                html: T.milestoneEmail(MOCK.name, { completed: 5, total: 18, milestone: '5 guides completed — 28% done' }) },
    reengagement:          { subject: 'Ready to continue your investing journey?',     html: T.reengagementEmail(MOCK.name) },
    promotion:             { subject: 'Upgrade to Plainvest Pro',                      html: T.promotionEmail({ subject: 'Upgrade to Plainvest Pro', headline: 'Take your investing further.', body: 'Pro gives you live portfolio tracking, 20-year projections, and your Freedom Score — all in one place.', ctaText: 'Upgrade to Pro', ctaUrl: 'https://plainvest.app', subtext: 'Available as a monthly subscription. Cancel anytime.' }) },
    sub_cancelled:         { subject: 'Your Pro subscription has been cancelled',      html: T.subscriptionCancelledEmail(MOCK.name) },
    sub_renewed:           { subject: 'Your Pro subscription renewed',                 html: T.subscriptionRenewedEmail(MOCK.name, { price: MOCK.amount, nextRenewal: MOCK.nextRenewal }) },
    payment_failed:        { subject: 'Payment issue — Plainvest',                     html: T.paymentFailedEmail(MOCK.name) },
    email_verification:    { subject: 'Verify your email — Plainvest',                 html: T.emailVerificationEmail(MOCK.name, 'https://members.plainvest.app/auth/callback?token=test') },
